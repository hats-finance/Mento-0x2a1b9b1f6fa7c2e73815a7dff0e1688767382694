...
- Duration: 2 weeks
- Maximum Reward: $15,998.46
- Submissions: 45
- Total Payout: $1,559.85 distributed among 3 participants.
...
- **Add a disableInitialize mechanism to prevent Locking.sol frontrunning vulnerabilities**

  After deploying the `Locking.sol` contract, the `__Locking_init` function can be frontrun by an attacker, allowing them to initialize the contract with malicious parameters. It is suggested to implement a `disableInitialize` mechanism in the constructor to prevent this vulnerability and ensure proper initialization by legitimate parties.
...
- **Incorrect Storage Slot Gap Calculation in LockingBase.sol Contract**
  ...
  The `LockingBase.sol` contract incorrectly calculates storage slot gaps, creating risks for future upgrades. A recent update adjusted gaps from 50 to 47 slots without accounting for previously used slots, leading to a total of 59 slots where 50 is standard. This discrepancy could lead to storage collisions and contract malfunctions. Despite highlighting improvement needs, adjusting these slots now may shift storage layout, causing issues. The contribution has been recognized as valuable, deserving a low-level acknowledgment with a bonus.


  **Link**: [Issue #12](https://github.com/hats-finance/Mento-0x2a1b9b1f6fa7c2e73815a7dff0e1688767382694/issues/12)


- **Mento Locking Mechanism Vulnerability Allows Bypassing Minimum Amount Check**

  The Mento locking mechanism, derived from Rarible contracts, has a flaw where minimum lock amounts aren't enforced during relocking as they are in initial locks. This oversight allows bypassing of the minimum amount validation. A proposed solution is to ensure the sum of the residue and new amount meets the minimum required lock value.
...
The Hats finance audit competition for Mento highlighted key insights into bolstering security for decentralized web3 applications. Mento, which operates on the Celo blockchain, engaged in this decentralized audit to safeguard its protocols, especially focusing on its Locking Contracts. The audit, spanning two weeks with a maximum reward of $15,998.46, prompted 45 submissions with a payout of $1,559.85 to three participants. Participants were tasked with scrutinizing recent modifications to accommodate Celo’s transition to L2, ensuring consistent governance voting periods, user protection, and enhanced flexibility. It was integral to verify the proper handling of block times and governance functions under the new system configuration. The audit discovered several low-severity issues including a potential frontrunning vulnerability in contract initialization, improper storage slot gap calculations, and a flaw allowing bypassing of minimum lock amounts upon relocking. These findings underscore the necessity for ongoing, dynamic evaluations and improvements in smart contract security mechanisms.
